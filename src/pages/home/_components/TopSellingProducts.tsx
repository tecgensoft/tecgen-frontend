      ...
      <div className="container my-10 py-4 px-5 bg-white drop-shadow-md rounded-md">
        <div>
          <div className="flex justify-between items-center mb-3 h-10">
            <h1 className="text-xl py-5 font-medium text-black-dim flex items-center gap-3">
              <Image src={icon} alt="fc" className="w-6" /> Top Selling Products
            </h1>
            <Button className="underline font-bold text-secondary">
              See more
            </Button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {isLoading &&
              skeletonArray(5).map((_, index) => (
                <div
                  key={index}
                  className="w-[228px] h-[300px] bg-skeleton"
                ></div>
              ))}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
